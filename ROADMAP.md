...
schema
id
kind            # implementation（默认）| research（M-d 引入，创建后不可变）
objective
baseline_commit
read_scope
write_allow
...
risk_triggers
gates
depends_on      # 必须先完成的任务 ID（M-d 依赖边）
```

（注：早期草案的 `mode`（small/medium/large 风险级别）未作为 `TaskDefinition` 字段落地；
风险通过 `risk_triggers` + `--quick` 快速通道表达。）
...
```text
ctl adapter capabilities --adapter omp
...
本次写入若撞上其他活动任务的 `write_allow` 则硬拒；并补全此前为桩的 `ctl schedule validate` /
`ctl schedule run`（现均已落地）。
...
（`run.rs`）、`RunEventStore`（`.ctl/runs/`）、`control.run-state.v1` schema、`ctl agent-report`
...
### M6：run-scoped capability lease 接线 ✅ 已实现 — start_run 授予并消费原生 lease（`capability-lease-run-wiring-v1`）

**方向**：闭合 M-f 留后续里点名的「完整 capability lease（M6 主体）」子集。此前 `LeaseState` 与 lease
事件（`lease_created`/`lease_used`/`lease_revoked`/`lease_expired`）只活在 **task aggregate**（M4 任务内嵌
run 路径），M6 `start_run` 仅 `generate_uuid()` 盖一个不透明 `lease_id`，跨 run 重叠安全完全靠 run aggregate
而非 lease。这次把 run aggregate 自己的 lease 生命周期接上。

**已落地**：
- **typed lease 迁移**——`domain/lease.rs` 暴露 `LeaseState::grant(LeaseGrant)` / `consume` / `revoke` /
  `expire`（返回 typed `LeaseError`，**不解析 `Event`**）；task 与 run 两个 reducer 各自把 payload 解析成
  typed 入参后委托同一套迁移，task 错误文案保持字节一致。`LeaseState` 增 serde-default 绑定字段
  `task_id`/`adapter`/`scopes`（旧 task lease 投影不破）。
- **run aggregate lease**——`AgentRunState.lease: Option<LeaseState>`；`apply_run` 新增 `lease_created`/
  `lease_used`/`lease_revoked`。run 绑定不变量：`max_uses >= 2`（保证起跑消费一次后仍 Active；**task lease
  旧语义 `max_uses>=1` 不变**）、`task_id`/`adapter` 与 run 一致、**`scopes` 与 run `write_allow` 完全相等**
  （V1）。`run_started` 在存在原生 lease 时要求 `lease_id` 匹配 + Active + `remaining_uses>0`；legacy run
  （无 `lease_created`）仍按不透明 `lease_id` 放行。
- **start_run 授予并消费**——在既有 registry+per-run 临界区内依次落 `lease_created` → `lease_used`
  （起跑消费一次，`remaining_uses=99`）→ `run_started`，并把同一 `lease_id` 盖进 run manifest。**不要求外部
  预授予 lease**；**不声称 worktree/manifest 文件系统副作用与账本追加构成原子**——崩溃窗口由只读
  `ctl run recover`（孤儿 worktree / Queued 持 lease 的 partial-start）披露，绝不自动消解。
- **终止撤销**——`finish`/`fail`/`abort` 在终态事件前对仍 Active 的原生 lease 落 `lease_revoked`。
- **recover 只读披露**——`run.json` 投影与 `ctl run recover`（人类 LEASE/USES 列 + `--json`）暴露结构化
  `lease_status`（`ACTIVE`/`REVOKED`/`EXPIRED`，legacy → `UNKNOWN` + `lease_compat=pre_lease_run`）、
  `remaining_uses`、`lease_stale`（墙钟 TTL 超时**仅报告不追加 `lease_expired`**）、`lease_nonactive`、
  `partial_starts`。无散文式状态串。
- **复用 M4 默认值经共享常量**——`RUN_LEASE_TTL_SECONDS=3600` / `RUN_LEASE_MAX_USES=100`，M4 与 M6 同源。
- 测试——18 条新增（负例为主）：lease 迁移（grant/consume/expire-at-0/revoke 后拒用）；run reducer
  （lease_id 不匹配、`max_uses<2`、`scopes≠write_allow`、revoke 后拒用、legacy 可回放、fixture 确定性
  99 uses）；应用层（授予+消费、重叠拒绝**不落 lease 事件**、二次 start 不重复消费、finish 撤销并解锁重叠
  run、legacy→`UNKNOWN`、partial-start 只读检测、Running 持 revoked lease 标 `nonactive`）。

**仍留后续（未实现）**：每次实际文件写入消费一次 lease use（V1 只在 start 消费一次）、跨账本原子性、
加密能力、自动续跑——均按「禁止过早进入自动路径」留待真实需求。**未改冻结 schema**：run aggregate 事件
沿用 `Event::is_valid()` + reducer 校验（既有 run 事件即如此），不走信封 per-type schema。

## V1 认知层与工作流脚手架（record-and-disclose，均已实现）

硬层（M-a…M-g）之上的一组 record-only / 只读能力。它们**从不**门控 create/finish、不打分、
不替代审核独立性——只记录与披露，把判断权留给人与硬层。设计细节见
[EPISTEMIC_CONTROL.md](./EPISTEMIC_CONTROL.md)。

- **`ctl brainstorm`（来源溯源）✅** — `record / attach-critic / skip-critic / show`：记录任务派生自哪些
  发散/收敛认知工件。record-only，不声称思考质量或评审独立性。
- **`ctl uncertainty`（未知项账本）✅** — `record / evidence / dispose / status`：登记任务携带的未知项及
  其处置（`resolved` / `accepted_as_assumption` / `invalidated`）。`model` oracle 仅顾问性——命令层拒绝
  以 model 证据 resolve 一个未知项。
...
为不破坏「单一事实源」而刻意未接线、但 reducer 已实现并测试的前瞻状态机：

- **任务级多 run 镜像**：`run_scheduled` / `run_launched` / `run_merged` 事件 +
  `TaskState.active_runs` / `RunRef` / `schedule_plan_id` 的 reducer 分支已就绪并测试，但 `ctl schedule run`
  走的是 **AgentRun 聚合**（run-store）这条唯一事实源路径，不再镜像到任务级——接线会造成两份冲突状态。
...
`ctl-review` 依赖 `review-contract.md`，`ctl-diagnose` 依赖 `failure-diagnosis.md`。
**已采用方案二**：这些固定规则文件随仓库维护于 `.omp/spec/guides/`，并经
`src/infrastructure/skills.rs` 的 `include_str!` 嵌进 `ctl` 二进制，由 `ctl init` 注入
...
```powershell
ctl init
# 三步法（create/ready/start），或用 `ctl task quick` 一步融合：
ctl task create --id fix-config-parsing --objective "fix config parsing" `
  --read-scope src/config.rs --write-allow src/config.rs --gates cargo_test
ctl task ready --id fix-config-parsing
ctl task start --id fix-config-parsing
ctl context build --id fix-config-parsing
# 人或任意 AI 工具完成修改
ctl gate run --id fix-config-parsing --gate cargo_test
ctl task submit --id fix-config-parsing
ctl task finish --id fix-config-parsing
ctl task archive --id fix-config-parsing
ctl board
```
...
- 每个成功命令输出建议的下一步命令。
- `ctl board` 给人看，`ctl board --json` 给脚本和 adapter 看。
- 所有写操作支持 `--dry-run`。
