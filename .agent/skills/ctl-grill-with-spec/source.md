...
(and for protected paths blocks) it. Hand the confirmed note to `ctl task create` (or `ctl prd init` for a multi-task effort); a
...
to `ctl task create` (or `ctl prd init` for a multi-task effort); a durable lesson to `ctl-spec`.
...
AskUserQuestion — ask the whole **frontier** as ONE call carrying N separate
single-select questions (one per independent decision; a question depending on a
still-open one waits for a later round), each with the recommended answer listed
first and marked "(Recommended)". Reserve `multi: true` for the rare decision
where several options genuinely apply at once. When a frontier question needs a
repo fact, fire the read-only lookup (subagent or direct read) in the SAME turn
as the AskUserQuestion — non-blocking; only questions downstream of the
unresolved fact wait (for an instant local read, do it inline before asking).
Record which cognitive artifacts the eventual
