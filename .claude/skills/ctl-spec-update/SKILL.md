...
description: "Capture knowledge from debugging, implementation, or discussion into two tiers: project facts into .ctl/spec/, stable cross-project preferences into ~/.ctl/memory/ (global, adapter-referenced). Auto-triggered by control-guard after a task finishes (ctl task finish), by the Stop-hook wrap-up reminder, or after a ctl-oracle diagnosis reveals a pattern worth preserving."
---
...
- After a Bayesian diagnosis (e.g. via the `ctl-oracle` subagent) identifies a root cause that future sessions should avoid
...
control-guard routes here after `ctl task finish` succeeds: when the finished
task revealed a non-obvious pattern, capture it before the session ends.
Diagnosis on Claude Code runs through the read-only `ctl-oracle` subagent
(`.claude/agents/ctl-oracle.md`) rather than a `ctl-diagnose` skill — when its
root-cause analysis surfaces something future sessions should avoid, record it
here. Writing under `.ctl/spec/` is only permitted when that path is inside the
active task's `write_allow`; the `.claude/hooks/ctl-gate.py` PreToolUse gate
enforces it. If `.ctl/spec/` is out of scope, print the proposed update and let
the user place it or open a task for it.
