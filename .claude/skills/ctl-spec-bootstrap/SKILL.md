...
- You notice the specs are stale relative to recent code changes — Part 2

**Supports**: Rust, TypeScript/JavaScript (frontend + Node), Java (Maven/Gradle), Go, Python, and mixed-language projects.

> Scope: this skill writes only under `.ctl/`, `.trellis(.bak)/`, `.claude/`, `CLAUDE.md`, `AGENTS.md`. It never edits project source. See "Operating boundary" below.
...
- `.claude/` — control-plane skills, hooks, settings
...
This skill writes only inside its operating boundary (`.ctl/`, `.trellis(.bak)/`,
`.claude/`, `CLAUDE.md`, `AGENTS.md`); the `.claude/hooks/ctl-gate.py` PreToolUse
gate still governs every write, so the relevant paths must be in the active
task's `write_allow` (or run the migration before a task binds the boundary, when
only `.ctl/` and the entry files are touched).

**No automatic spec-drift detection on Claude Code.** Unlike the OMP
`ctl-context.ts` hook (which warns on `agent_end` / `session_shutdown` when code
changed but `.ctl/spec/` did not), the Claude integration ships only
`.claude/hooks/ctl-context.py` — a **SessionStart** hook that injects the active
task's boundaries. It does not track spec freshness. So on Claude Code, run
`/ctl-spec-bootstrap` **explicitly**: once when introducing ctl to the project,
and again after a refactor large enough to invalidate the specs. The
companion `ctl-spec-update` skill captures incremental knowledge after a task
finishes; this skill is the full (re)generation pass.
