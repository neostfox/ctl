...
## Embedded Skill/Hook Registration

The control plane ships its `.omp/` skills and hooks by compiling them into the `ctl`
binary and writing them out on `ctl init`. There is one registration point.

**Single registration point**: `src/infrastructure/skills.rs` (`all_embedded_files()`)

When adding a new skill or hook under `.omp/`:

1. Add an `EmbeddedFile { relative_path, content: include_str!("../../.omp/...") }` entry.
2. Done. `cmd_init` / `inject_all()` iterate that list directly; there is no second list.

**Why this matters**: a skill not listed in `all_embedded_files()` exists in this repo but
is never distributed by `ctl init`. New skills and bug fixes silently fail to reach
downstream projects. Do not reintroduce a hand-maintained second list; iterate
`all_embedded_files()` as the single source of truth.

### Quick checklist for a new skill

```bash
# After adding .omp/skills/<name>/SKILL.md, verify it is registered:
grep -n "<name>/SKILL.md" src/infrastructure/skills.rs   # should match
cargo build                                              # include_str! fails loudly on a wrong path
```
