...
```text
.ctl/tasks/<task>/events.jsonl  # append-only truth
.ctl/tasks/<task>/task.json     # replay projection
```
...
|---|---|
| Missing `.ctl/tasks` root | `control init` creates it; `open` commands ask to run init. |
...
- Good: `control task create --id good --objective ok --read-scope src --write-allow src --gates cargo_check` writes `.ctl/tasks/good/events.jsonl` and `.ctl/tasks/good/task.json`.
...
- Unit: store writes per-task `.ctl/tasks/<task>/events.jsonl` and projection `task.json`.
...
.ctl/tasks/<id>/  # M1 canonical task ledgers and projections
```
