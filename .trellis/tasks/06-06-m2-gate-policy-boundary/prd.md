...
- [ ] `gate run` executes only allowlisted command templates with bounded output and no network access.
- [ ] `context build` produces deterministic hash snapshot of read scope files.
- [ ] Boundary check detects files modified outside `write_allow` and records violations.
- [ ] Unknown gates fail schema/app/reducer checks.
- [ ] `task start` transitions Planning → InProgress.
- [ ] Agent skill updated to auto-run gates and context build.
